...
| `guide/service-and-cost-comparison.md` | 用途検索と費用baselineの方法 |
| `config/cloud-perspectives.json` | feedへ付与するcloud別perspective |
| `scripts/build_feed_digest.py` | 公式feed→静的intelligence |
| `site/data/service-catalog.json` | 用途→4cloud候補、比較質問、cost scenario |
| `site/data/cost-baselines.json` | 同条件の公開単価、region、SKU、除外項目 |
...
6. 費用比較は基準日、region、SKU、formula、除外項目、公式sourceを必須にする。
7. capacity-matchを性能同等と表現しない。採用前にworkload benchmarkを行う。
